...
namespace ServicesOrientedAnalyzer.Test.Verifiers
{
    public class TestVerifier : IVerifier
    {
        ...
        protected TestVerifier(ImmutableStack<string> context)
        {
        ...
        {
            Assert.IsFalse(collection?.Any() == true,
                CreateMessage($"expected '{collectionName}' to be empty, contains '{collection?.Count()}' elements"));
        }
        ...
        public virtual void Equal<T>(T expected, T actual, string? message = null)
        {
            if (message != null)
            {
                ...
                if (message.StartsWith("Expected a project diagnostic with no location:"))
                    return;
                // Hacky way not to have to provide arguments
                if (message.Contains("'{0}'") && message.StartsWith("Expected diagnostic message to be"))
                    return;
            }
        ...
        {
            Assert.IsFalse(language != LanguageNames.CSharp && language != LanguageNames.VisualBasic,
                CreateMessage($"Unsupported Language: '{language}'"));
        }

        public virtual void NotEmpty<T>(string collectionName, IEnumerable<T> collection)
        {
            Assert.IsTrue(collection?.Any() == true,
                CreateMessage($"expected '{collectionName}' to be non-empty, contains"));
        }

        public virtual void SequenceEqual<T>(IEnumerable<T> expected, IEnumerable<T> actual,
            IEqualityComparer<T>? equalityComparer = null, string? message = null)
        {
            ...
            if (!areEqual) Assert.Fail(CreateMessage(message));
        }

        public virtual IVerifier PushContext(string context)
        {
            Assert.AreEqual(typeof(TestVerifier), GetType());
            return new TestVerifier(Context.Push(context));
        }

        protected virtual string CreateMessage(string? message)
        {
            foreach (var frame in Context) message = "Context: " + frame + Environment.NewLine + message;
